...
use crate::{Error, MpcSignatureDelivered};
...
impl From<TransactionReceipt> for EthRecipt {
    fn from(value: TransactionReceipt) -> Self {
        EthRecipt {
            transaction_hash: format!("{:?}", &value.transaction_hash),
            transaction_index: value.transaction_index.as_u64(),
            block_hash: value.block_hash.map(|x| format!("{:?}", x)),
            block_number: value.block_number.map(|x| x.as_u64()),
            from: format!("{:?}", value.from),
            to: value.to.map(|x| format!("{:?}", &x)),
            cumulative_gas_used: value.cumulative_gas_used,
            gas_used: value.gas_used,
            contract_address: value.contract_address.map(|x| format!("{:?}", x)),
            // logs: value.logs,
